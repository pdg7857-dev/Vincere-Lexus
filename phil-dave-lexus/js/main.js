...
(function () {
  ...
  function buildNewsletter() {
    ...
    garage.forEach(function (room) {
      ...
      form.addEventListener("submit", function (e) {
        ...
        if (cfg.endpoint) {
          // no-cors + text/plain avoids the CORS preflight Apps Script can't
          // answer (same pattern as the lead forms); we proceed optimistically
          try { data.captured_at = new Date().toISOString(); } catch (err) {}
          try {
  ...
  window.__pdLexusSpin = function (id, sectionEl) {
    ...
    (lexSpin ? Promise.resolve(lexSpin) : import("./lexusspin.js?v=21").then(function (m) { lexSpin = m; return m; }))
      .then(function (m) {
  ...
  function loadLexusTool() {
    ...
    mount.__loaded = true;
    fetch("lexus-2026.html?v=21").then(function (r) { return r.text(); }).then(function (txt) {
