...
export function start(host, cfg) {
  ...
  return loadModel(cfg.spinModel).then(function (src) {
    ...
    if (cfg.spinHide && cfg.spinHide.length) {
      g.traverse(function (o) {
        if (!o.isMesh) return;
        var names = [o.name || ""];
        (Array.isArray(o.material) ? o.material : [o.material]).forEach(function (m) { if (m && m.name) names.push(m.name); });
        if (cfg.spinHide.some(function (sub) { return names.some(function (n) { return n.indexOf(sub) !== -1; }); })) o.visible = false;
      });
    }
    spin.add(prepare(g, cfg.spinLength));
