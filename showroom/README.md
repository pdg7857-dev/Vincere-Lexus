...
typeface and **Source Sans Pro** body font (bundled in `showroom/fonts/`), Lexus's
...
```
https://raw.githack.com/pdg7857-dev/vincere-lexus/<commit-sha>/showroom/index.html
```
...
The app is a static site with the dataset baked into `showroom/data.js`, so it works by
just opening the file — no build step, no server required:

```
open showroom/index.html     # or double-click it
```

Or serve it (recommended for tablets / sharing on the showroom Wi-Fi):

```
python3 -m http.server 8099 --directory showroom
...
python3 scripts/build_dataset.py   # normalize -> data/lexus.json + showroom/data.js
```
...
scripts/build_dataset.py   # normalize + join -> data/lexus.json, showroom/data.js
data/raw/                  # raw API responses (regenerable)
data/lexus.json            # canonical normalized dataset
showroom/                  # the tool (index.html, styles.css, app.js, data.js)
```
